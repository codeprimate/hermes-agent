...
class MatrixAdapter(BasePlatformAdapter):
    ...
    async def _on_room_message(self, event: Any) -> None:
        ...
        # Bot message / m.notice filtering:
        # MATRIX_ALLOW_BOTS (none|mentions|all) is the fork control.
        # MATRIX_PROCESS_NOTICES=true bridges into "all" when allow_bots is unset/none.
        if msgtype == "m.notice":
            mode = getattr(self, "_allow_bots", "none") or "none"
            if mode == "none" and getattr(self, "_process_notices", False):
                mode = "all"
            if mode == "none":
                return
            # "mentions" lets through; mention gating runs in _resolve_message_context
